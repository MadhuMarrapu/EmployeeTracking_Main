...
public class PersonServiceImpl implements PersonService {
	...
	public PersonDTO create(PersonDTO dto) {
		...
		person.setConfirmPassword(encoded);
	...
	public List<PersonDTO> getAllResponses() {
		return personRepo.findByStatusFlag(Status.ACTIVE).stream().map(this::convertToDTO).toList();
	}

	@Override
	public PersonDTO getByIdResponse(Integer id) {
		return personRepo.findById(id).filter(p -> p.getStatusFlag() == Status.ACTIVE).map(this::convertToDTO)
				.orElseThrow(() -> new PersonNotFoundException(PERSON_NOT_FOUND));
	}

	@Override
	public List<PersonDTO> getByRoleResponse(Roles role) {
		return personRepo.findByRole(role).stream().filter(p -> p.getStatusFlag() == Status.ACTIVE)
	...
	public Person getPersonEntity(String email) {
		return personRepo.findByEmail(email).filter(p -> p.getStatusFlag() == Status.ACTIVE)
